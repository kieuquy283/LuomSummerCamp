...
const highlights = [
  {
    icon: <MapPin className="w-6 h-6 text-brand-cyan mb-2" />,
    label: 'Địa điểm',
    value: 'Trường THCS Tô Hiệu',
  },
  {
    icon: <Calendar className="w-6 h-6 text-brand-electric mb-2" />,
    label: 'Thời gian',
    value: '3 buổi/tuần • 3 tuần',
  },
  {
    icon: <Rocket className="w-6 h-6 text-brand-yellow mb-2" />,
    label: 'Bắt đầu',
    value: 'Tháng 7',
  },
  {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="text-center lg:text-left">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white leading-[1.6] mb-6">
              Dự án giáo dục{' '}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-cyan to-brand-electric">
                Nâng cao năng lực số
              </span>
            </h1>

            <p className="text-lg md:text-xl text-slate-300 mb-8 max-w-2xl mx-auto lg:mx-0 leading-relaxed">
              Đội Bình dân học vụ số phường Vĩnh Phúc tìm kiếm tình nguyện viên đồng hành chiến dịch mùa hè.
              Mang kiến thức công nghệ và tư duy số đến gần hơn với học sinh THCS!
            </p>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center lg:justify-start space-y-4 sm:space-y-0 sm:space-x-4 mb-12">
              <a
                href="#tuyen-giang-vien"
                className="w-full sm:w-auto inline-flex items-center justify-center px-8 py-3.5 bg-brand-yellow hover:bg-brand-yellow-hover text-white font-bold rounded-full shadow-[0_0_20px_rgba(251,191,36,0.4)] transition-all hover:-translate-y-1"
              >
                Vị trí tìm kiếm
              </a>
              <a
                href="#loi-ich"
                className="w-full sm:w-auto px-8 py-3.5 bg-transparent hover:bg-white/5 border border-slate-500 hover:border-white text-white font-medium rounded-full transition-all flex items-center justify-center group"
              >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-3xl mx-auto lg:mx-0">
              {highlights.map((item) => (
                <div
                  key={item.label}
                  className="w-full min-h-[132px] bg-white/5 border border-white/10 rounded-2xl px-5 py-5 backdrop-blur-sm flex flex-col items-center justify-center text-center hover:bg-white/10 transition-colors"
                >
                  {item.icon}
                  <span className="text-xs text-slate-400 font-medium">{item.label}</span>
                  <span className="text-sm text-white font-semibold mt-1">{item.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="relative h-[500px] w-full hidden lg:block">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-72 h-72 border border-brand-cyan/30 rounded-full flex items-center justify-center animate-[spin_20s_linear_infinite]">
              <div className="w-56 h-56 border border-brand-electric/40 rounded-full flex items-center justify-center animate-[spin_15s_linear_infinite_reverse]"></div>
            </div>
            ...
            <div
              className="absolute top-[30%] right-[15%] bg-brand-navy/90 backdrop-blur-sm border border-brand-yellow/30 p-4 rounded-tl-2xl rounded-tr-2xl rounded-bl-2xl shadow-xl transform translate-x-4 -translate-y-4 z-20 animate-bounce"
